...
/proc/sanitizeSQL_a0(t as text)
	t = replacetext(t, "я", "&#1103;")
	var/sqltext = dbcon.Quote(t);
	return copytext(sqltext, 2, lentext(sqltext)); //Quote() adds quotes around input, we already do that and fix "я"
...
/proc/stripped_input(mob/user, message = "", title = "", default = "", max_length=MAX_MESSAGE_LEN)
	...
	name = replacetext(name, "я", "___255_")
	...
	name = replacetext(name, "___255_", "я")
	return name
