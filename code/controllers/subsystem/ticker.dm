...
/datum/subsystem/ticker/fire()
	switch(current_state)
		if(GAME_STATE_STARTUP)
			...
			text2world("<b><font color='blue'>Welcome to the pre-game lobby!</font></b>")
			text2world("Please, setup your character and select ready. Game will start in [config.lobby_countdown] seconds")
...
/datum/subsystem/ticker/proc/setup()
	...
	if(master_mode == "random" || master_mode == "secret")
		...
		if(!mode)
			if(!runnable_modes.len)
				text2world( "<B>Unable to choose playable game mode.</B> Reverting to pre-game lobby.")
				return 0
	...
	if(!Debug2)
		if(!can_continue)
			qdel(mode)
			mode = null
			text2world("<B>Error setting up [master_mode].</B> Reverting to pre-game lobby.")
			SSjob.ResetOccupations()
			return 0
	else
	...
	if(hide_mode)
		...
		text2world("<B>The current game mode is - Secret!</B>")
		text2world("<B>Possibilities:</B> [english_list(modes)]")
	else
		mode.announce()
			...
			if(Player.stat != DEAD && !isbrain(Player))
				...
				if(station_evacuated) //If the shuttle has already left the station
					if(!Player.onCentcom() && !Player.onSyndieBase())
						Player.text2tab("<font color='blue'><b>You managed to survive, but were marooned on [station_name()]...</b></FONT>")
					else
						num_escapees++
						Player.text2tab("<font color='green'><b>You managed to survive the events on [station_name()] as [Player.real_name].</b></FONT>")
				else
					Player.text2tab("<font color='green'><b>You managed to survive the events on [station_name()] as [Player.real_name].</b></FONT>")
			else
				Player.text2tab("<font color='red'><b>You did not survive the events on [station_name()]...</b></FONT>")
