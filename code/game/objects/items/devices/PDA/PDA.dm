				...
				if(in_range(src, U) && loc == U)
					if(t)
						if(hidden_uplink && (trim(lowertext(t)) == trim(lowertext(lock_code))))
							...
							U.text2tab("The PDA softly beeps.")
							U << browse(null, "window=pda")
							src.mode = 0
						else
...
/obj/item/device/pda/proc/send_message(mob/living/user = usr,list/obj/item/device/pda/targets)
	...
	for(var/obj/item/device/pda/P in targets)
		...
		if(MS)
			...
			if(!multiple)
				user.text2tab("<span class='notice'>ERROR: Server isn't responding.</span>")
				return
	photo = null
...
/obj/item/device/pda/proc/show_recieved_message(datum/data_pda_msg/msg,obj/item/device/pda/source)
	...
	if(L && L.stat != UNCONSCIOUS)
		L.text2tab("\icon[src] <b>Message from [source.owner] ([source.ownjob]), </b>\"[msg.message]\"[msg.get_photo_ref()] (<a href='byond://?src=\ref[src];choice=Message;skiprefresh=1;target=\ref[source]'>Reply</a>)")
...
/obj/item/device/pda/proc/show_to_ghosts(mob/living/user, datum/data_pda_msg/msg,multiple = 0)
	for(var/mob/M in player_list)
		if(isobserver(M) && M.client && (M.client.prefs.chat_toggles & CHAT_GHOSTPDA))
			...
			M.text2tab("[link] <span class='name'>[msg.sender] </span><span class='game say'>PDA Message</span> --> <span class='name'>[multiple ? "Everyone" : msg.recipient]</span>: <span class='message'>[msg.message][msg.get_photo_ref()]</span></span>")
...
/obj/item/device/pda/AltClick()
	...
	if(usr.canUseTopic(src))
		if(id)
			...
			usr.text2tab("<span class='warning'>This PDA does not have an ID in it!</span>")
...
/obj/item/device/pda/verb/verb_remove_pen()
	...
	if (usr.canUseTopic(src))
		...
		if(O)
			...
			usr.text2tab("<span class='warning'>This PDA does not have a pen in it!</span>")
...
/obj/item/device/pda/attackby(obj/item/C, mob/user, params)
	if(istype(C, /obj/item/weapon/cartridge) && !cartridge)
		...
		if(!user.unEquip(C))
			return
		cartridge.loc = src
		user.text2tab("<span class='notice'>You insert [cartridge] into [src].</span>")
		if(cartridge.radio)
		...
		if(!idcard.registered_name)
			user.text2tab("<span class='warning'>\The [src] rejects the ID!</span>")
			return
		if(!owner)
		...
		C.loc = src
		pai = C
		user.text2tab("<span class='notice'>You slot \the [C] into [src].</span>")
		updateUsrDialog()
	else if(istype(C, /obj/item/weapon/pen))
		var/obj/item/weapon/pen/O = locate() in src
		if(O)
			user.text2tab("<span class='warning'>There is already a pen in \the [src]!</span>")
		else
			if(!user.unEquip(C))
				return
			C.loc = src
		...
		photo = P.img
		user.text2tab("<span class='notice'>You scan \the [C].</span>")
	else
		return ..()
...
/obj/item/device/pda/afterattack(atom/A as mob|obj|turf|area, mob/user, proximity)
		...
		if(3)
			if(!isnull(A.reagents))
				if(A.reagents.reagent_list.len > 0)
					...
					user.text2tab("<span class='notice'>[reagents_length] chemical agent[reagents_length > 1 ? "s" : ""] found.</span>")
					for (var/re in A.reagents.reagent_list)
						user.text2tab("<span class='notice'>\t [re]</span>")
				else
					user.text2tab("<span class='notice'>No active chemical agents found in [A].</span>")
			else
				user.text2tab("<span class='notice'>No significant chemical agents found in [A].</span>")
	...
	if (!scanmode && istype(A, /obj/item/weapon/paper) && owner)
		...
		if (!PP.info)
			user.text2tab("<span class='warning'>Unable to scan! Paper is blank.</span>")
			return
		notehtml = PP.info
