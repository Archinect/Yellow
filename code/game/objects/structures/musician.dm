	...
	if(href_list["newsong"])
		...
		spawn()
			...
			if(copytext(lines[1],1,6) == "BPM: ")
				...
				tempo = sanitize_tempo(5) // default 120 BPM
			if(lines.len > 50)
				usr.text2tab("Too many lines!")
				lines.Cut(51)
			var/linenum = 1
			for(var/l in lines)
				if(lentext(l) > 50)
					usr.text2tab("Line [linenum] too long!")
					lines.Remove(l)
				else
					linenum++
...
/obj/structure/piano/attackby(obj/item/O, mob/user, params)
	if (istype(O, /obj/item/weapon/wrench))
		if (!anchored && !isinspace())
			...
			user.text2tab("<span class='notice'> You begin to tighten \the [src] to the floor...</span>")
			...
			playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
			user.text2tab("<span class='notice'> You begin to loosen \the [src]'s casters...</span>")
