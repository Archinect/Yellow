	...
	if(href_list["choice"])
		if(istype(inserted_id))
			...
			if(istype(I))
				...
				I.loc = src
				inserted_id = I
			else usr.text2tab("<span class='danger'>No valid ID.</span>")
...
/obj/machinery/mineral/equipment_vendor/proc/RedeemVoucher(obj/item/weapon/mining_voucher/voucher, mob/redeemer)
	var/selection = input(redeemer, "Pick your equipment", "Mining Voucher Redemption") as null|anything in list("Survival Capsule and Survival Pen", "Resonator and Advanced Scanner", "Mining Drone", "Medivac Kit", "Extraction Kit")
...
/obj/item/weapon/card/mining_point_card/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/weapon/card/id))
		if(points)
			...
			user.text2tab("<span class='info'>You transfer [points] points to [C].</span>")
			points = 0
		else
			user.text2tab("<span class='info'>There's no points left on [src].</span>")
	..()

/obj/item/weapon/card/mining_point_card/examine(mob/user)
	..()
	user.text2tab("There's [points] point\s on the card.")
